...
const SubjectsShow = () => {
    ...
    return (
        <ShowView className="class-view space-y-6">
            <ShowViewHeader resource="subjects" title={subjectDetails.subject.name} />
            <Card className="hover:shadow-md transition-shadow">
                <CardHeader className="flex w-full flex-row items-center justify-between">
                    <CardTitle>Subject Overview</CardTitle>
                    <Badge variant='secondary'>{subjectDetails.subject.code}</Badge>
                </CardHeader>

                <CardContent className="space-y-3">
                    <p className="text-sm text-muted-foreground">
                        {subjectDetails.subject.description ?? "No description provided."}
                    </p>
                </CardContent>
            </Card>
            ...
            <Card className="hover:shadow-md transition-shadow">
                ...
                <CardContent className="space-y-2">
                    {subjectDetails.subject.department ? (
                        <>
                            <Link
                                to={`/departments/show/${subjectDetails.subject.department.id}`}
                                className='text-lg font-semibold text-foreground hover:underline'
                            >
                                {subjectDetails.subject.department.name}
                            </Link>
                            <p className="text-sm text-muted-foreground">
                                {subjectDetails.subject.department.description ?? "No department description provided."}
                            </p>
                        </>
                    ) : (
                        <p className="text-sm text-muted-foreground">
            ...
            <Card className="hover:shadow-md transition-shadow">
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <Badge variant="secondary">{subjectDetails.totals.classes}</Badge>
                </CardHeader>
